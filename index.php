<?php
/* ================================================================
   index.php — Stazione Meteo, Rovigo - Viola Marchesini
   Backend PHP + pagina HTML in un unico file.
================================================================ */
...
elseif (isset($json_body['action'])) $action = $json_body['action'];

/* ── Se e' una chiamata API rispondi JSON e termina ─────────── */
if ($action !== '') {
    ...
    try {
        $dsn    = 'mysql:host='.DB_HOST.';dbname='.DB_NAME.';charset=utf8mb4;connect_timeout='.DB_TIMEOUT;
        $opts   = [
        ...
        if (defined('PDO::MYSQL_ATTR_CONNECT_TIMEOUT')) {
            $opts[PDO::MYSQL_ATTR_CONNECT_TIMEOUT] = DB_TIMEOUT;
        }
    ...
    $body = $json_body; /* gia' letto sopra */

    switch ($action) {

        case 'ping':
            jout(['ok'=>true, 'db'=>$db_ok, 'db_error'=>$db_ok?null:$db_error,
                  'session'=>isset($_SESSION['user'])?$_SESSION['user']:null]);

        case 'check':
            jout(['ok'=>!empty($_SESSION['user']), 'user'=>isset($_SESSION['user'])?$_SESSION['user']:null, 'db'=>$db_ok]);

        case 'login':
            if (!$db_ok) jout(['error'=>'DB_UNAVAILABLE','db'=>false], 503);
            $u = trim($body['username'] ?? '');
            $p = $body['password'] ?? '';
            if (!$u || !$p) jout(['error'=>'Username e password richiesti'], 400);
            ...
            $valid = password_verify($p, $user ? $user['password_hash'] : $dummy) && $user;
            if (!$valid) {
        ...
        case 'create_user':
            need_admin();
            if (!$db_ok) jout(['error'=>'Database non disponibile'], 503);
            $nu = trim($body['username'] ?? '');
            $np = $body['password'] ?? '';
            $nr = in_array($body['role']??'',['admin','user'],true) ? $body['role'] : 'user';
            if (strlen($nu)<3)  jout(['error'=>'Username troppo corto (min 3)'], 400);
            if (strlen($nu)>64) jout(['error'=>'Username troppo lungo (max 64)'], 400);
            if (!preg_match('/^[a-zA-Z0-9_.\-]+$/',$nu)) jout(['error'=>'Username non valido'], 400);
            if (strlen($np)<8)  jout(['error'=>'Password troppo corta (min 8)'], 400);
            try {
...
<div class="login-overlay" id="loginOverlay">
    <div class="login-card">
        <div class="login-logo"><i class="fas fa-cloud-sun"></i></div>
        ...
        <div class="admin-body">
            <section class="admin-section">
                ...
                <div class="create-user-row">
                    <input class="field-input sm" type="text" id="newUsername" placeholder="Username (min 3)">
                    <input class="field-input sm" type="password" id="newPassword" placeholder="Password (min 8)">
...
<div class="container" id="appContainer" style="display:none">
            ...
            <div class="header-right">
                ...
                <div class="header-actions">
                    ...
                    <button class="btn-icon header-btn" id="adminBtn"
                        style="display:none" title="Pannello Admin">
                        <i class="fas fa-users-cog"></i>
                    </button>
                    <button class="theme-toggle" id="tempUnitToggle">&deg;C</button>
                    <button class="theme-toggle" id="themeToggle">
                        <i class="fas fa-moon"></i>
                    </button>
                    <button class="btn-icon header-btn logout-btn" id="logoutBtn" title="Logout">
    ...
    <div class="error-banner" id="errorBanner">
        ...
        <button class="close-error" id="closeError"><i class="fas fa-times"></i></button>
    </div>
                ...
                <div class="weather-card temperature-card">
                    ...
                    <div class="card-body">
                        <div class="main-value" id="temperature">
                            <span class="value">--</span><span class="unit">&deg;C</span>
                        </div>
                        <div class="sub-info">
                            <div class="sub-item">
                                <i class="fas fa-arrow-up"></i><span id="tempMax">--</span>
                            </div>
                            <div class="sub-item">
                                <i class="fas fa-arrow-down"></i><span id="tempMin">--</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="charts-grid">
                <div class="chart-container">
                    <h3 class="chart-title">
                        <i class="fas fa-chart-line"></i> Temperatura (24h)
                    </h3>
                    <canvas id="temperatureChart"></canvas>
                </div>
                <div class="chart-container">
                    <h3 class="chart-title">
                        <i class="fas fa-chart-area"></i> Umidita &amp; Pressione (24h)
                    </h3>
    ...
    <footer class="footer">
        <p>&copy; 2026 Stazione Meteo - Rovigo, Viola Marchesini</p>
    </footer>
</div>
